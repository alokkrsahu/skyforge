...
./t1_sitl.sh [N] [arena]  # N drones (default 4); arena = Gazebo world (default 'default')
./t1_sitl.sh -h           # list available arenas + caveats
./t1_sitl.sh 8 walls      # e.g. 8 drones in the 'walls' world
...
### Arena / World (arena-agnostic)

**Selecting an arena.** `./t1_sitl.sh [N] [arena]` sets `PX4_GZ_WORLD=<arena>` → PX4 loads
`<arena>.sdf` from `$PX4_GZ_WORLDS` (default `~/src/PX4-Autopilot/Tools/simulation/gz/worlds`).
`./t1_sitl.sh -h` lists every `.sdf` there with caveats. Only `t1` takes the arena.

**Auto-detection (no hardcoded world name).** The gz transport namespace is `/world/<inner
name>/…`, where the inner `<world name>` is NOT always the filename (`frictionless.sdf` and our
`default.sdf`/`forest_stage.sdf` are all inner-named `default`). So `t2`, `t1`'s model-remove, and
the runtime LED backends **discover** the running world name instead of assuming it:
- Runtime: `runtime/show/gz_world.py` `resolve_gz_world()` — `$SKYFORGE_GZ_WORLD` override →
  parse `gz topic -l` for the first `/world/<name>/clock` → fallback `"default"`. Cached, bounded
  (2 s), never raises; the LED backends (`led_backend.py`) and `drone_lights.py` build
  `/world/<name>/…` from it. Lazy (first LED call) so it never delays arming.
- `t1`: after "Gazebo world is ready", detects the inner name into `GZ_WORLD` for `/world/$GZ_WORLD/remove`.
- `t2`: readiness poll matches any `/world/*/clock`.
With no arena + no env, every path resolves `"default"` → byte-for-byte the old behavior.

**The `default` arena is a forest-clearing stage** generated by
`runtime/worlds/make_forest_stage.py` (repo copy: `runtime/worlds/forest_stage.sdf`; plain gray
saved as `default.sdf.plain.bak`). It is the only DART@100 Hz world; **stock worlds are ODE** and
may crash >~40 drones (warned, not blocked); `forest`/`baylands`/`underwater` pull Fuel models.
