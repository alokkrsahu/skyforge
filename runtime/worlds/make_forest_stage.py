...
(The runtime is arena-agnostic now — `t2`, `t1`'s model-remove, and the LED backends auto-detect
the running world name — so the inner ``<world name>`` need not be ``default``. We keep it
``default`` here only because this world IS the deployed ``default.sdf`` / the default arena.)
...
<sdf version="1.9">
  <!-- Skyforge 'forest clearing' stage. Generated by runtime/worlds/make_forest_stage.py.
       Inner name 'default' because this IS the default arena (default.sdf); the runtime
       auto-detects the world name, so it isn't load-bearing. DART@100Hz for big fleets.
